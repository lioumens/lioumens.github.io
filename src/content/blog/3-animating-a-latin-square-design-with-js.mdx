...
* Always use `viewBox` combined with `preseveAspectRatio`.
...
* [Rachel Nabors](http://rachelnabors.com/) is a cartoonist and web animations expert who also wrote the fantastic, pratical [introduction to WAAPI on the MDN web docs](https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API/Using_the_Web_Animations_API).
...
The code used in the Latin Square design animation is not all that different. There is some additional javascript code to figure out positioning depending on which label is being animatetd, and we also use the `offset` key to specify the timing of the keyframes. Finally, we use a `forEach` call to start the animation for each row label.
...
// the keyframing of the animation
   let labelPosition = [
